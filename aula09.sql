...
SELECT nome
FROM tbl_peca
WHERE preco > 10 AND cod_peca IN (
    SELECT cod_peca
    FROM tbl_estoque
);
...
EXCEPT
SELECT * FROM tbl_peca WHERE nome='PLACA'
order by nome;
...
from tbl_peca
where cod_peca IN (
select e.cod_peca from tbl_fornecedor f
...
INTERSECT
SELECT nome,cor FROM tbl_peca WHERE cidade='LONDRES'
...
SELECT quantidade FROM tbl_estoque
WHERE cod_peca=2)
