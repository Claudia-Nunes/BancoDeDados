...
CREATE OR REPLACE FUNCTION fc_reduz_qtde_projetos() 
RETURNS TRIGGER
language plpgsql AS 
$$
BEGIN
	UPDATE tbl_departamentos SET qtde_projetos=qtde_projetos-1 
	WHERE cod_departamento = OLD.cod_departamento;
	RETURN OLD;
END 
$$ ;
CREATE OR REPLACE TRIGGER tr_reduz_qtde_projetos
BEFORE DELETE ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION fc_reduz_qtde_projetos();


--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado
CREATE OR REPLACE FUNCTION fc_aumenta_qtde_projetos() 
RETURNS TRIGGER
language plpgsql AS 
$$
BEGIN
...
END 
$$ ;
CREATE OR REPLACE TRIGGER tr_aumenta_qtde_projetos
AFTER INSERT ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION fc_aumenta_qtde_projetos();
...
CREATE TABLE tbl_empregado_historico_salario (cod_empregado int, salario real, data timestamp);
CREATE OR REPLACE FUNCTION fc_empregado_historico_salario() 
RETURNS TRIGGER
language plpgsql AS 
$$
BEGIN
	INSERT INTO tbl_empregado_historico_salario VALUES(OLD.cod_empregado, OLD.salario, now());
	RETURN NEW;
END 
$$ ;
CREATE OR REPLACE TRIGGER tr_empregado_historico_salario
BEFORE UPDATE ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION fc_empregado_historico_salario();
...
ALTER TABLE IF EXISTS tbl_empregados
    ADD COLUMN data_atualizacao timestamp ;

CREATE OR REPLACE FUNCTION fc_data_atualizacao() 
RETURNS TRIGGER
language plpgsql AS 
$$
BEGIN
...
CREATE OR REPLACE TRIGGER tr_data_atualizacao
BEFORE UPDATE ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION fc_data_atualizacao();
