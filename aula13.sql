...
BEGIN
	...
	WHILE (i<=qtde_empregados) LOOP
		...
		i:=i+1;
	END LOOP;
	SELECT te.nome INTO nome FROM tbl_empregados as te WHERE te.cod_empregado=codigo_empregado;
	RETURN nome;
END $$ ;
...
DECLARE
	resultado real;
BEGIN
	SELECT avg(salario) INTO resultado FROM tbl_empregados;
	RETURN resultado;
END
$$ ;
...
CREATE OR REPLACE FUNCTION fc_salarios(codigo int, OUT sal real, OUT sal10 real, OUT sal15 real)
language plpgsql AS 
$$
BEGIN
	SELECT te.salario INTO sal FROM tbl_empregados as te
	WHERE te.cod_empregado = codigo;
	sal10 := sal * 1.10;
...
BEGIN
	RETURN QUERY
	SELECT tp.cod_projeto, tp.nome FROM tbl_projetos as tp;
	RETURN;
END
$$ ;
...
DECLARE
	resultado int;
BEGIN
	SELECT count(tp.cod_projeto) INTO resultado
	FROM tbl_projetos as tp
	WHERE tp.cod_departamento = codigo;
	RETURN resultado;
END
$$ ;
